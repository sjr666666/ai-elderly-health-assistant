...
核心亮点：**RAG 用药知识库**（936 条知识：药品 / 慢病指南 / FAQ）——AI 回答基于检索资料生成，**可溯源、降幻觉**；SSE 流式输出 + 药箱个性化（AI 知道老人真实在吃什么药）+ 三级离线降级（向量 → 关键词 → 本地直出），医疗场景离线可用；**Function Calling 工具调用**——AI 听懂老人需求后真正办事（查药箱 / 建计划 / 标记漏服 / 通知家属），从问答助手升级为 AI 管家。
...
- Flyway 数据库版本管理（V1~V9 增量迁移，无需手动 SQL）
- Hutool 工具库
- 阿里云 OSS（药品图片存储，可选）
- 百度 OCR（药品包装识别）
- 百度 TTS（语音播报）
- 百度 ASR（语音识别，老人语音问药；未配 Key 自动降级 Web Speech API）
- DeepSeek AI（紧急咨询 / 用药指导 / 冲突分析 / RAG 生成 / Function Calling 工具调用）
- **Function Calling**：AI 可调用系统工具（查药箱 / 建服药计划 / 标记漏服 / 通知家属），从"问答"升级为"行动"
- **SafetyGuard 安全防护**：prompt 注入 / 危险剂量请求拦截式拒绝（不调用 LLM）+ 统一免责声明
...
│   │   │   │   ├── ToolSpec               # 工具规格（触发话术单一真源，description/引导自动生成）
│   │   │   │   └── impl/AiToolServiceImpl # 工具执行实现（药箱/计划/短信）
